            ...
            if (cell->is_locally_owned())
              {
                ...
                bool adjacent_present = false;
                bool weakzone_present = false;
                ...
                for (unsigned int p=0; p<quadrature.size(); ++p)
                  {
                    ...
                    if (prelim_composition_values[adjacent_refinement[0]][p] > 0.1)
                      {
                        adjacent_present = true;
                        if (prelim_composition_values[adjacent_refinement[0]][p] >= 1.0)
                        {
                          in_center_of_compo = true;
                        }
                      }
                    if (prelim_composition_values[weakzone_refinement[0]][p] > 0.01)
                      {
                        weakzone_present = true;
                        if (prelim_composition_values[weakzone_refinement[0]][p] >= 1.0)
                        {
                          in_center_of_compo = true;
                        }
                      }
                ...
                //Only continue if at least one is true
                    ...
                    else if (weakzone_present)
                      {
                        minimum_refinement_level = weakzone_refinement[1];
                        maximum_refinement_level = weakzone_refinement[2];
                      }
                    ...
                    else if (adjacent_present)
                      {
                        minimum_refinement_level = adjacent_refinement[1];
                        maximum_refinement_level = adjacent_refinement[2];
                      }
                    else
                      {
      ...
      prm.enter_subsection("Mesh refinement");
      {
        prm.enter_subsection("Composition");
        {
          ...
          prm.declare_entry("Overriding plate refinement","",
                            ...
                            "its maximum refinement level.");
          prm.declare_entry("Adjacent plate refinement","",
                            Patterns::List (Patterns::Integer(0)),
                            "The compositional field number of the adjacent plate, its minimum refinement level and "
                            "its maximum refinement level.");
          prm.declare_entry("Weakzone refinement","",
                            Patterns::List (Patterns::Integer(0)),
                            "The compositional field number of the weak zone, its minimum refinement level and "
                            "its maximum refinement level.");
      ...
      prm.enter_subsection("Mesh refinement");
      {
        ...
        prm.enter_subsection("Composition");
        {
          ...
          const std::vector<int> adjacent
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Adjacent plate refinement")));

          adjacent_refinement = std::vector <unsigned int> (adjacent.begin(),
                                                              adjacent.end());

          AssertThrow (adjacent_refinement.size() == 3,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (adjacent_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (adjacent_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the adjacent plate cannot be "
                                   "smaller than the minimum level of the whole model. "));

          AssertThrow (adjacent_refinement[2] <= max_level,
                       ExcMessage ("The maximum refinement for the adjacent plate cannot be "
                                   "greater than the maximum level of the whole model. "));

          const std::vector<int> weakzone
            = Utilities::string_to_int(
                Utilities::split_string_list(prm.get("Adjacent plate refinement")));

          weakzone_refinement = std::vector <unsigned int> (weakzone.begin(),
                                                              weakzone.end());

          AssertThrow (weakzone_refinement.size() == 3,
                       ExcMessage ("The number of refinement data given here must be "
                                   "equal to 3 (field number + min level + max level). "));

          AssertThrow (weakzone_refinement[0] < this->n_compositional_fields(),
                       ExcMessage ("The number of compositional field to refine (starting "
                                   "from 0) should be smaller than the number of fields. "));

          AssertThrow (weakzone_refinement[1] >= min_level,
                       ExcMessage ("The minimum refinement for the weakzone plate cannot be "
                                   "smaller than the minimum level of the whole model. "));

          AssertThrow (weakzone_refinement[2] <= max_level,
                       ExcMessage ("The maximum refinement for the weakzone plate cannot be "
                                   "greater than the maximum level of the whole model. "));
          ...
          AssertThrow (crust_refinement[0] != slab_mantle_refinement[0] && \
                       ...
                       crust_refinement[0] != adjacent_refinement[0] && \
                       crust_refinement[0] != weakzone_refinement[0] && \
                       slab_mantle_refinement[0] != overriding_refinement[0] && \
                       slab_mantle_refinement[0] != adjacent_refinement[0] && \
                       slab_mantle_refinement[0] != weakzone_refinement[0] && \
                       overriding_refinement[0]  != adjacent_refinement[0], 
...
namespace aspect
{
  namespace MeshRefinement
  {
    ASPECT_REGISTER_MESH_REFINEMENT_CRITERION(TagComposition,
                                              ...
                                              "and min refinement levels are set for the crustal "
                                              "field, the individual plates and the mantle. ")
  }
}
